...
class ResidualClassificationWrapper(BaseEstimator):
    """Predict binary target as prior rate + residual adjustment from a regressor."""
    ...
    def _prior_series(self, X):
        if not isinstance(X, pd.DataFrame):
            raise TypeError(
                f"ResidualClassificationWrapper expects a DataFrame with {self.prior_col}."
            )
        return pd.to_numeric(X[self.prior_col], errors="coerce")
    ...
    def predict(self, X):
        prior = self._prior_series(X).fillna(self.fallback_)
        residual_hat = self.regressor.predict(self._feature_matrix(X))
        return (prior.to_numpy() + residual_hat >= 0.5).astype(int)


def build_catboost_residual_expanding_clf(
    iterations=300,
    depth=6,
    learning_rate=0.05,
    l2_leaf_reg=3.0,
    **kwargs,
    ...
    del kwargs
    return ResidualClassificationWrapper(
        _make_catboost_regressor(
            iterations=iterations,
            depth=depth,
            learning_rate=learning_rate,
