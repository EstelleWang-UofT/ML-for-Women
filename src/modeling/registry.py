...
from modeling.models.multiclass import (
    build_catboost_history_multiclass,
    build_lightgbm_multiclass,
    build_rf_multiclass,
)
...
MULTICLASS_MODELS = {
    "lightgbm": build_lightgbm_multiclass,
    "random_forest": build_rf_multiclass,
}

HISTORY_MULTICLASS_MODELS = {
    "catboost_history": build_catboost_history_multiclass,
}

RESIDUAL_MULTICLASS_MODELS = {
    "catboost_residual_expanding": build_catboost_residual_expanding,
}
...
HISTORY_TREE_MULTICLASS_MODELS = {"catboost_history"}
RESIDUAL_TREE_MULTICLASS_MODELS = {"catboost_residual_expanding"}
...
def _catboost_history_multiclass_search_space(trial):
    return {
        **_catboost_search_space(trial),
        "ewma_alpha": trial.suggest_float(
            "ewma_alpha", EWMA_ALPHA_RANGE[0], EWMA_ALPHA_RANGE[1]
...
def is_history_tree_model(name, task="ordinal"):
    if task == "multiclass":
        return name in HISTORY_TREE_MULTICLASS_MODELS
    return name in HISTORY_TREE_ORDINAL_MODELS


def is_residual_tree_model(name, task="ordinal"):
    if task == "multiclass":
        return name in RESIDUAL_TREE_MULTICLASS_MODELS
...
def resolve_training_data(name, bundle, task="ordinal"):
    ...
    if name in HISTORY_TREE_MULTICLASS_MODELS and task == "multiclass":
        return {
    ...
    if name in RESIDUAL_TREE_MULTICLASS_MODELS and task == "multiclass":
        return {
            "y_train_val": y_train_val,
            "y_test": y_test,
            "groups": bundle.groups_train_val,
...
def get_search_space(name, task="ordinal"):
    ...
    if task == "multiclass" and name == "catboost_history":
        return _catboost_history_multiclass_search_space
    return spaces[name]
